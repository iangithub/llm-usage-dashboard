...
// MM-DD in *local* time. toISOString() is UTC, which shifts the label a full
// day backwards for timezones east of UTC (e.g. UTC+8).
function mmdd(ms) {
  const d = new Date(ms);
...
function dailySeries(events, now, days) {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    buckets.push({ dayStart, label: mmdd(dayStart), total: 0, cost: 0 });
  }
...
function buildModel({ codex, claude, antigravity, now = Date.now() }) {
  ...
  const antigravityProvider = antigravity ? {
    active: antigravity.active || false,
    stale: antigravity.stale || false,
    cachedAt: antigravity.cachedAt || null,
    message: antigravity.message || '',
    name: antigravity.name || '',
    email: antigravity.email || '',
    planName: antigravity.planName || '',
    promptCredits: antigravity.promptCredits || null,
    flowCredits: antigravity.flowCredits || null,
    models: antigravity.models || [],
    daily: Array(14).fill(0).map((_, i) => {
      const dayStart = startOfDay(now) - (13 - i) * 86400000;
      return { dayStart, label: mmdd(dayStart), total: 0, cost: 0 };
    })
  } : { active: false, stale: false, models: [] };

  return {
    generatedAt: now,
    providers: { codex: codexProvider, claude: claudeProvider, antigravity: antigravityProvider },
